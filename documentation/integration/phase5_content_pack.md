...
**Sub-headline:** PillSafe is a medication-safety assistant for seniors and Canadians with
...
### How PillSafe Works (4 step cards)

- **01 · Scan Your Prescription** — Photograph your prescription. PillSafe reads the
...
- **02 · Confirm Your Medications** — Each medication is matched against Health Canada's
  Drug Identification Numbers (DINs). You confirm with one tap — PillSafe never guesses on
  your behalf.
- **03 · Verify a Pill** — Photograph a loose pill on the PillSafe capture card. Its colour,
...
Section intro line: *Every pill check ends in one of three honest outcomes. When PillSafe
...
- **Needs a Closer Look** (amber) — PillSafe isn't sure yet. It may ask you to flip the pill
...
- **Doesn't Match** (red) — This pill doesn't match anything in your profile. A clear warning,
  because a stray pill is exactly what PillSafe exists to catch.
- **Nothing Detected** (navy/neutral) — No pill found in the photo, with capture tips to try
  again.

Under-grid line: *PillSafe is tuned so that a wrong pill being called "verified" is the
...
Intro: *PillSafe's design decisions rest on peer-reviewed evidence — including the evidence
...
1. **NLM Pill Image Recognition Challenge (2016)** — IEEE AIPR — Even the winning system
   ...
   identification is the wrong task for a safety app — so PillSafe verifies against your
   profile instead.
2. **ePillID (Usuyama et al., 2020)** — CVPR Workshops — The benchmark for fine-grained pill
   ...
   from. PillSafe's attribute-based design avoids depending on per-pill image galleries.
3. **GO-PILL (2025)** — MDPI Mathematics — Reading the tiny imprint pressed into a pill is
   the hardest and most decisive step. PillSafe reads every imprint twice, with two
...
**Body:** PillSafe is a capstone research project. It supports your decisions — it does not
...
## 2. `/about` — About PillSafe

**Title:** About PillSafe
...
**Body (section: What PillSafe Is):**
PillSafe helps two groups the medication system underserves: seniors managing many
...
**Scope note:** PillSafe is a Conestoga College capstone project, built for Canada
...
**Values (4 cards):**
- **Safety before convenience** — When evidence is thin, PillSafe abstains. An honest "I'm
...
- **Honesty about limits** — PillSafe is research-grade decision support with mandatory
...
that is not a safety tool. PillSafe's reframing: don't identify against everything — verify
...
comprehension drops, adherence and safety drop with it. PillSafe treats translation into the
user's own language — grounded in the official monograph, never free-styled — as a
first-class safety function.

**Closing line:** PillSafe exists for the moment a hand hesitates over an open pill
...
**Lede:** Every design decision in PillSafe traces to published evidence or to our own
measured experiments — including the negative results.

### Section A — Published evidence PillSafe stands on (cards; each = citation + why it matters)
...
1. **Yaniv et al. — The NLM Pill Image Recognition Challenge (IEEE AIPR 2016)** — The
   ...
   the challenge winner achieved only 43% top-5 accuracy. This is why PillSafe verifies
...
2. **Zeng, Cao & Zhang — MobileDeepPill (ACM MobiSys 2017)** — The NLM challenge winner,
   ...
   carry complementary signal, the same attribute decomposition PillSafe uses.
3. **Usuyama et al. — ePillID (CVPR Workshops 2020)** — Established the low-shot reality of
   pill recognition: most products have almost no training photos. PillSafe's text-attribute
...
4. **Ling et al. — Few-Shot Pill Recognition (CVPR 2020)** — Reinforces the few-shot framing
   ...
   motivation for PillSafe's controlled capture card.
5. **GO-PILL (MDPI Mathematics 2025)** — Documents that low-contrast, debossed imprints are
   ...
   imprint is the decisive, hardest attribute — PillSafe reads it twice with complementary
...
6. **MedSnap ID (medRxiv 2020; related US patents)** — Prior commercial art pairing a capture
   ...
   patient-profile verification. PillSafe cites it as prior art; our open, ablated capture-card
...
PillSafe's pill-vision pipeline is itself a research contribution in preparation. The paper
...
be drafted from those results. Until then, PillSafe's measured performance figures remain
...
**Note under grid:** PillSafe is a capstone project of the Conestoga College graduate program
...
- **Greeting (FR):** Bonjour ! Je suis l'Assistant PillSafe. Je peux vous expliquer comment
  PillSafe fonctionne — la vérification des comprimés, la conception axée sur la sécurité,
...
- **Out-of-scope fallback (EN):** I'm not sure that's something I know — I can answer
  questions about the PillSafe project: how it works, its safety design, the research, and
...
- **Out-of-scope fallback (FR):** Je ne suis pas certain de pouvoir répondre à cela — je peux
  répondre aux questions sur le projet PillSafe : son fonctionnement, sa conception axée sur
...
truthful that PillSafe is a capstone and decision-support only.
