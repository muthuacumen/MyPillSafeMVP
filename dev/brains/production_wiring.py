...
#: `app.py`/`config.py`. `<repo>`'s parent is where `IMB1_Prototype/` lives
#: as a sibling of the repo itself -- the SAME sibling-of-repo-parent
#: convention `config.py` already uses for IMB1_ROOT/SB2_ROOT/BB3_ROOT.
_PILLSAFE_PARENT = Path(__file__).resolve().parents[3]
NB08_SRC = _PILLSAFE_PARENT / "IMB1_Prototype" / "NB08_Notebook" / "src"
