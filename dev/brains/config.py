...
# --- RE-LANDED 2026-08-18 (owner order, "Only A3+A4c is needed"): the
#     2026-08-15 removal below was ARMED at the ops window, crashed the
#     sidecar process twice on its first scan (STATUS_ACCESS_VIOLATION), and
#     was ROLLED BACK the same day -- see
#     archive\2bdeleted\2026-08-15_paddleocr_removal\MANIFEST.md for the full
#     rollback record. The legacy paddle dual-read then served mypillsafe.ca
#     again until this date. The crash's root cause was never diagnosed, so
#     this re-land is a scope decision (drop the legacy arm entirely), not a
#     claim that the earlier crash is fixed.
#
# `PILLSAFE_READER` -- master switch for `/pill/analyze`'s imprint path.
#   "two_stage" (DEFAULT, since 2026-08-15, re-landed 2026-08-18): route
#          through `production_wiring.analyze()`, which builds ONE
#          `VerifySession` and runs A3 presence gate -> A4c constrained read.
#          This is the ONLY arm (owner decision 2026-08-15, reaffirmed
#          2026-08-18) -- there is no fallback reader to degrade to.
#   "off"  RETIRED 2026-08-15, retirement RE-LANDED 2026-08-18. Used to
#          select the legacy `imb1.analyze_pill(photo)` paddle OCR dual-read
#          subprocess path. That code no longer exists in `IMB1_v0/imb1/` --
#          see `archive\2bdeleted\2026-08-15_paddleocr_removal\MANIFEST.md`
#          for the archived source and restore instructions. Setting
...
if READER_MODE not in _VALID_READER_MODES:
    if READER_MODE in _RETIRED_READER_MODES:
        raise RuntimeError(
            ...
            "2026-08-15, retirement re-landed 2026-08-18 (archived at "
...
# UPDATED 2026-08-15 (GOAL1), re-landed 2026-08-18: there is no longer any
# switch that avoids the scorer -- `PILLSAFE_READER=off` used to, but that
# value is retired above (legacy paddle OCR path removed). The two-stage
# reader is the only arm.
