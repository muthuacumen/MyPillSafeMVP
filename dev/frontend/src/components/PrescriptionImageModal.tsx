...
export default function PrescriptionImageModal({ prescriptionId, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    prescriptionsApi
      ...
      .catch(() => setError(t('rxImage.error')));
    return () => {
    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
          <h3 className="font-semibold text-slate-900">{t('rxImage.title')}</h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-4 flex items-center justify-center min-h-[200px]">
          ...
          {objectUrl && <img src={objectUrl} alt={t('rxImage.alt')} className="max-w-full rounded-lg" />}
        </div>
      </div>
    </div>
  );
